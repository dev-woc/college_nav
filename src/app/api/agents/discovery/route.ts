...
// POST /api/agents/discovery — trigger the College Discovery Agent for the current student
export async function POST(request: NextRequest) {
	...
	if (userProfile.role !== "student") {
		return NextResponse.json(
			{ error: "Only students can run the discovery agent" },
			{ status: 403 },
		);
	}
	...
	if (!userProfile.onboardingCompleted) {
		return NextResponse.json(
			{ error: "Complete onboarding before running the discovery agent" },
			{ status: 400 },
		);
	}
	...
	try {
		const summary = await runDiscoveryAgent(studentProfile.id);
